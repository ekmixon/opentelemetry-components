		...
		for j := 0; j < ilmsAct.Len(); j++ {
			...
			for k := 0; k < metricsAct.Len(); k++ {
				...
				case pdata.MetricDataTypeSum:
					...
					assertEqualDoubleDataPointSlice(t, metricAct.Name(), metricAct.Sum().DataPoints(), metricExp.Sum().DataPoints())
				case pdata.MetricDataTypeIntGauge:
					assertEqualIntDataPointSlice(t, metricAct.Name(), metricAct.IntGauge().DataPoints(), metricExp.IntGauge().DataPoints())
				case pdata.MetricDataTypeGauge:
					assertEqualDoubleDataPointSlice(t, metricAct.Name(), metricAct.Gauge().DataPoints(), metricExp.Gauge().DataPoints())
				default:
